...
class Signals:
    pattern_value = re.compile(r"([a-z]+|\d+) -> ([a-z]+)")
    pattern_not = re.compile(r"NOT ([a-z]+) -> ([a-z]+)")
    pattern_and = re.compile(r"([a-z]+|\d+) AND ([a-z]+) -> ([a-z]+)")
    pattern_or = re.compile(r"([a-z]+) OR ([a-z]+) -> ([a-z]+)")
    pattern_lshift = re.compile(r"([a-z]+) LSHIFT (\d+) -> ([a-z]+)")
    pattern_rshift = re.compile(r"([a-z]+) RSHIFT (\d+) -> ([a-z]+)")
