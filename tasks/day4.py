...
class Room:
    ...
    def static_initialization(cls):
        if not cls._initialized:
            ...
            cls._alphabet = [chr(i + cls._first_char) for i in range(cls._last_char - cls._first_char + 1)]
    ...
    def decrypt(self):
        ...
        for char in self._enc_name:
            if char != '-':
                char = chr((ord(char) - self._first_char + self.room_id) % self._mod + self._first_char)
            new_name += char
        return new_name
    ...
    with open(data, 'r') as f:
        for line in f.readlines():
            ...
            if room.is_valid():
                ...
                if all([check in true_name for check in ['north', 'pole', 'object', 'storage']]):
                    print('{}: {}'.format(
                        true_name,
                        room.room_id
                    ))
