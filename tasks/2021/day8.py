...
def puzzle2():
    ...
    entries = list(filter(bool, DATA.split("\n")))
    entries = list(
        map(lambda x: (x.split(" | ")[0].split(), x.split(" | ")[1].split()), entries)
    )
    ...
    def decode(values: Iterable[str]) -> Dict[str, int]:
        ...
        result[3] = next(
            filter(lambda x: sum(k in x for k, v in counts_5 if v == 2) == 2, len_5)
        )
    ...
    for first, second in entries:
        ...
        total += int(
            "".join(map(str, (decoder["".join(sorted(value))] for value in second)))
        )
