...
def puzzle1():
    ...
    required = frozenset(['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid', ])  # 'cid' - optional
    passport_fields = set()
    valid_count = 0
        ...
        if entry == '':
            if passport_data and required.issubset(passport_data):
                ...
                if not hgt[-2:] in ('cm', 'in'):
                    is_valid = False
                if hgt.endswith('cm') and not (hgt[:-2].isdigit() and 150 <= int(hgt[:-2]) <= 193):
                    is_valid = False
                if hgt.endswith('in') and not (hgt[:-2].isdigit() and 59 <= int(hgt[:-2]) <= 76):
                    is_valid = False
