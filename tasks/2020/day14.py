    ...
    for entry in entries:
        ...
        if cmd.startswith('mask'):
            code.append((
                Cmd.MASK,
                (
                    sum(0 if bit == '0' else 2 ** index for index, bit in enumerate(reversed(value))),
                    sum(2 ** index if bit == '1' else 0 for index, bit in enumerate(reversed(value))),
                ),
            ))
...
def puzzle2():
    ...
    def replace_multiple(source, old: str, new: Iterable[str]):
        ...
        source = source.replace('0', '.').replace('1', '-')  # this needs for correct submasks
        for n in new:
    ...
    for entry in entries:
        ...
        if cmd.startswith('mask'):
            code.append((
                ...
                (
                    (
                        sum(0 if bit == 'X' else 2 ** index for index, bit in enumerate(reversed(value))),
                        sum(2 ** index if bit == '1' else 0 for index, bit in enumerate(reversed(value))),
                    ),
                    [
                        (
                            sum(0 if bit == '0' else 2 ** index for index, bit in enumerate(reversed(
                                replace_multiple(value, 'X', seq)
                            ))),
                            sum(2 ** index if bit == '1' else 0 for index, bit in enumerate(reversed(
                                replace_multiple(value, 'X', seq)
                            )))
                        )
    ...
    def print_it(val):
        return ''.join(reversed(list(map(str, (
            int(bool((2 ** i) & val))
            for i in range(36)
        )))))
